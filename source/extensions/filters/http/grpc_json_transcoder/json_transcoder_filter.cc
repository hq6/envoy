...
void AsyncReflectionFetcher::startReflectionRpcs() {
  ...
  for (std::string service: services_) {
    ...
    // TODO: Make this timeout part of the configuration
    ...
    if (server_reflection_method == nullptr) {
      // TODO: Remove this.
      std::cerr << "Failed to find MethodDescriptor for ServerReflectionInfo." << std::endl;
...
void DescriptorPoolBuilder::requestDescriptorPool(JsonTranscoderConfig* json_transcoder_config) {
  ...
  if (async_reflection_fetcher_ == nullptr) {
    ...
    // TODO: Ensure this calls loadFileDescriptorProtos or throws an
    // EnvoyException eventually.
  }
}
